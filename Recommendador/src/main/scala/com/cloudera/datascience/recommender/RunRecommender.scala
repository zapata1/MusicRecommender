...
class RunRecommender(private val spark: SparkSession, private var userId: Int) {
  ...
  def preparation(
    ...
    //Affiche les 5 premieres ligne
    rawUserArtistData.take(5).foreach(println)


    //regroupe par user et artist dans un dataFrame
    val userArtistDF = rawUserArtistData.map { line =>
      val Array(user, artist, _*) = line.split(' ')
      (user.toInt, artist.toInt)
    }.toDF("user", "artist")

    //nous montre le tableau
    userArtistDF.agg(min("user"), max("user"), min("artist"), max("artist")).show()

    //Qu'est ce que ca fait ca ok met en
    val artistByID = buildArtistByID(rawArtistData) //Frame
    val artistAlias = buildArtistAlias(rawArtistAlias) //Map[Int, Int]
    //on prend le bad et good id du premier artist
